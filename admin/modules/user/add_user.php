<?php 
	if(isset($_POST['sbm'])) {
		...
		$sqlCheckEmailExists = "SELECT * FROM users WHERE user_mail = '$user_mail'";
		$queryCheckEmailExists = mysqli_query($conn, $sqlCheckEmailExists);
		if(mysqli_num_rows($queryCheckEmailExists) > 0) {
			$errorEmailExists = '<div class="alert alert-danger">Email đã tồn tại !</div>' ;
		}else{
			//xử lý thêm dữ liệu
			$sqlInsertUser = "INSERT INTO users(user_full, user_mail, user_pass, user_level)
